...
def createBinarySearchTree(input_list = []):
    """
    构建二叉树
    :param input_list:
    :return:
    ...
    node = TreeNode(data)
    node.left = createBinarySearchTree(input_list)
    node.right = createBinarySearchTree(input_list)
...
root =createBinarySearchTree(my_input_list)
...
def preOrderTraversal_Stack(node):
    stack = []
